...
::  For command line switches, check https://10086678.xyz/command_line_switches.html
...
%nceline%
echo 不支持的版本。
echo 仅支持Windows 7/8/8.1/10/11和对应的Server版本。
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 系统中没有powershell.exe。
echo 中止...
goto MASend
)
...
%nceline%
echo 脚本正在从临时文件夹执行，
echo 您可能直接从压缩包中运行了该文件。
echo:
echo 请解压压缩包并从解压后的文件夹中执行。
goto MASend
)
)
...
echo:                 激活方式:
echo:
echo:             [1] HWID激活    ^|  Windows           ^|  永久激活
echo:             [2] KMS38激活   ^|  Windows           ^|  至2038年
echo:             [3] 在线KMS激活 ^|  Windows / Office  ^|     180天
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Green% "请输入菜单选项 [1,2,3,4,5,6,7,0] :"
choice /C:12345670 /N
set _erl=%errorlevel%

if %_erl%==8 exit /b
if %_erl%==7 start https://10086678.xyz & goto :MainMenu
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "请输入菜单选项 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%
...
echo:             [3] 在线KMS激活
echo:             
echo:             [4] HWID激活  ^(Windows^) ^+ 在线KMS激活 ^(Office^)
echo:             [5] KMS38激活 ^(Windows^) ^+ 在线KMS激活 ^(Office^)
echo:             __________________________________________________      
echo:                                                                   
echo:             [0] 返回
echo:       ______________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "请输入菜单选项:"
choice /C:123450 /N
set _erl=%errorlevel%
...
call :dk_color %Red% "启用 [sppsvc] 服务失败, 其他进程可能需要更多时间..."
echo:
)
...
echo:
echo 下载地址,
echo https://10086678.xyz/genuine-installation-media.html
goto dk_done
)
)
...
echo 正在查找系统信息                        [%winos% ^| %winbuild% ^| %arch%]
...
call :dk_color %Magenta% "下载地址： https://10086678.xyz/genuine-installation-media.html"
)
)
...
call :dk_color %Magenta% "[%altedition%] 版本激活码将用于HWID激活。"
echo:
)
...
call :dk_color %Red% "安装通用激活码          [%key%] [失败] %errorcode%"
if not defined error (
call :dk_color %Magenta% "进入故障排除选项并运行修复许可证选项。"
if defined actidnotfound call :dk_color %Red% "激活ID未发现，请确保您正在使用更新版本的脚本。"
set showfix=1
)
set error=1
)
...
if !errorlevel! EQU 0 (
echo 更改Windows区域为USA          [成功]
) else (
call :dk_color %Red% "更改Windows区域为USA          [失败]"
)
)
...
call :dk_color %Red% "生成GenuineTicket.xml            [失败]"
echo [%encoded%]
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :dl_final
) else (
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "安装GenuineTicket.xml            [重启ClipSVC服务失败, 等待中...]"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装GenuineTicket.xml            [clipup -v -o失败]"
)
...
call :dk_color2 %Magenta% "更多帮助：" %_Yellow% " https://10086678.xyz/licensing-servers-issue"
)
)
...
call :dk_color %Red% "删除注册表信息                     [失败] [!_ident!]"
) || (
echo 删除注册表信息                     [成功] [!_ident!]
)
...
call :dk_color %Magenta% "目前HWID激活不支持该版本。"
call :dk_color %Magenta% "请使用KMS38激活。"
) else (
if not defined error call :dk_color %Magenta% "进入故障排除选项并运行修复许可证选项。"
call :dk_color2 %Magenta% "更多帮助：" %_Yellow% " https://10086678.xyz/troubleshoot"
)
)
...
if %osSKU%==175 call :dk_color %Red% "%winos%不支持在非azure平台激活。"
...
if defined serv_e (
set error=1
call :dk_color %Red% "启用服务                       [失败] [%serv_e%]"
echo %serv_e% | findstr /i "ClipSVC-1058 sppsvc-1058" %nul% && (
call :dk_color %Magenta% "重启系统以便修复禁用服务错误error 1058."
)
)
...
set error=1
call :dk_color2 %Red% "检查启动模式                      " %Magenta% "[系统正在安全模式下运行。请在正常模式下运行。]"
)


reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Setup\State" 2>nul | find /i "IMAGE_STATE_COMPLETE" 1>nul || (
set error=1
call :dk_color2 %Red% "检查审核模式                     " %Magenta% "[系统正在审核模式下运行。请在正常模式下运行。]"
)


reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinPE" /v InstRoot %nul% && (
set error=1
call :dk_color2 %Red% "检查WinPE系统                          " %Magenta% "[系统正在WinPE系统下运行。请在正常模式下运行。]"
)


for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
call :dk_color %Gray% "检查WMIC.exe                       [未找到]"
)


%psc% $ExecutionContext.SessionState.LanguageMode 2>nul | find /i "Full" 1>nul || (
set error=1
call :dk_color %Red% "检查Powershell                     [无响应]"
)
...
set error=1
set wmifailed=1
call :dk_color %Red% "检查WMI                            [无响应] %_wmic%"
call :dk_color %Magenta% "进入故障排除选项并运行修复WMI选项。"
)


if not "%regSKU%"=="%wmiSKU%" (
call :dk_color %Red% "检查WMI/REG SKU                    [发现差别 - WMI:%wmiSKU% Reg:%regSKU%]"
)
...
if %error_code% NEQ 0 (
call :dk_color %Red% "检查DISM                           [无响应] %error_code%"
)


if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" (
set error=1
call :dk_color %Red% "检查Eval Packages                  [Eval Windows已安装非Eval许可证]"
)
...
if %error_code% NEQ 0 (
set error=1
call :dk_color %Red% "检查slmgr /dlv                     [无响应] %error_code%"
)


reg query "HKU\S-1-5-20\Software\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform\PersistedTSReArmed" %nul% && (
set error=1
call :dk_color2 %Red% "检查Rearm                          " %Magenta% "[系统需要重启]"
)


reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ClipSVC\Volatile\PersistedSystemState" %nul% && (
set error=1
call :dk_color2 %Red% "检查ClipSVC                        " %Magenta% "[系统需要重启]"
)
...
call :dk_color %Red% "检查SkipRearm                      [初始值0未找到，更改为0]"
...
call :dk_color %Red% "检查SPP Registry Key               [发现错误ModuleId]"
set error=1
)
...
if not defined applist (
set error=1
call :dk_color %Red% "检查Activation IDs                 [未找到]"
)
)
...
set error=1
call :dk_color %Red% "检查SPP Token Folder               [未找到] [%tokenstore%\]"
)


if exist "%tokenstore%\" if not exist "%tokenstore%\tokens.dat" (
set error=1
call :dk_color %Red% "检查SPP tokens.dat                 [未找到] [%tokenstore%\]"
)
...
set error=1
call :dk_color %Red% "检查sppsvc.exe                     [未找到]"
)
...
if defined wpaerror call :dk_color %Red% "检查WPA注册表键值              [发现错误] [共计 - !count!]"
)
...
if defined permerror call :dk_color %Red% "检查SPP Permissions                [发现错误]"

set showfix=1
call :dk_color %Magenta% "进入故障排除选项并运行修复许可证选项。"
if not defined permerror call :dk_color %Magenta% "如果激活仍然失败，请运行WPA Registry修复选项。"
)
...
call :dk_color %_Yellow% "按任意键返回到 %_exitmsg%..."
pause >nul
exit /b
...
::  5th column = Ticket signature value. It's as it is, it's not encoded. (Check https://10086678.xyz/hwid.html#Manual_Activation to see how it's generated)
...
%eline%
echo 不支持的版本。
echo KMS38激活支持Windows 10/11/Server, build 14393和更高版本。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "请输入菜单选项 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo 无法激活服务器评估版本，请转换版本。
echo:
echo 请进入更多选项，使用 '更改Windows版本' 选项.
) else (
echo 无法激活评估版本。
echo 请安装完整版本的 %winos%
echo:
...
echo 更多帮助：
echo https://10086678.xyz/kms38.html
goto dk_done
)
)
...
call :dk_color %Red% "为KMS38检查可替换版本    [%altedition% 未检测到激活ID]"
)
...
call echo 检查已安装的 Product Key          [Partial Key - %%_partial%%] [Volume:GVLK]
)
...
if !error_code! EQU 0 (
call :dk_refresh
echo 安装KMS客户端Setup Key         [%key%] [成功]
) else (
call :dk_color %Red% "安装KMS客户端Setup Key         [%key%] [失败] !error_code!"
if not defined error (
call :dk_color %Magenta% "进入故障排除选项并运行修复许可证选项。"
set showfix=1
)
...
if not defined app (
call :dk_color %Red% "检查已安装的GVLK激活ID   [未找到] 中止..."
goto :dk_done
)
...
echo 添加指定的KMS主机                [LocalHost 127.0.0.2] [成功]
) else (
call :dk_color %Red% "添加指定的KMS主机                [LocalHost 127.0.0.2] [失败]"
)
...
echo:
if exist "!_clipup!" (
echo 复制clipup.exe File到              [%systemroot%\System32\] [成功]
) else (
call :dk_color %Red% "复制clipup.exe File到              [%systemroot%\System32\] [失败] 中止..."
goto :k_final
)
)
...
::  Check https://10086678.xyz/kms38.html#Manual_Activation to see how it's generated
...
call :dk_color %Red% "停止sppsvc服务                 [失败]"
)
...
if %errorlevel%==0 (
echo 应用SKU-ID Rearm                   [成功]
) else (
call :dk_color %Red% "应用SKU-ID Rearm                   [失败]"
)
call :dk_refresh
...
%nul% reg delete "HKLM\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Magenta% "使用KMS保护KMS38                    [成功锁定注册表键值]"
) || (
call :dk_color %Red% "使用KMS保护KMS38                    [锁定注册表键值失败]"
)
)
...
call :dk_color %Red% "%winos% 不支持在非azure平台激活。"
)
...
%nceline%
echo 系统中没有wmic.exe。
if %winbuild% GEQ 22621 echo 请确保WMIC已启用。
goto Done
)
...
echo.              [1] 激活 - Windows
echo.              [2] 激活 - Office
echo.              [3] 激活 - Windows和Office
echo.
if defined _tskinstalled call :_color2 %_White% "              [4] 安装自动续期      " %_Green% "[已安装]"
if defined _oldtsk       call :_color2 %_White% "              [4] 安装自动续期      " %_Red% "[旧的安装]"
...
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 重写Office vNext     " %_Red% "[是]"
) else (
echo               [7] 重写Office vNext     [是]
)
) else (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 重写Office vNext     " %_Yellow% "[否]"
) else (
echo               [7] 重写Office vNext     [否]
)
)
echo.              _______________________________________________       
echo.
...
set "_mO21a=检测到Office 2021 C2R零售版已激活"
set "_mO19a=检测到Office 2019 C2R 零售版已激活 is activated"
set "_mO16a=检测到Office 2016 C2R 零售版已激活 is activated"
set "_mO15a=检测到Office 2013 C2R 零售版已激活 is activated"
set "_mO21c=检测到Office 2021 C2R 零售版不能转换为VOL批量许可版"
set "_mO19c=检测到Office 2019 C2R 零售版不能转换为VOL批量许可版"
set "_mO16c=检测到Office 2016 C2R 零售版不能转换为VOL批量许可版"
set "_mO15c=检测到Office 2013 C2R 零售版不能转换为VOL批量许可版"
set "_mO14c=检测到Office 2010 C2R 零售版不支持使用该脚本激活"
set "_mO14m=检测到Office 2010 MSI 零售版不支持使用该脚本激活"
set "_mO15m=检测到Office 2013 MSI 零售版不支持使用该脚本激活"
set "_mO16m=检测到Office 2016 MSI 零售版不支持使用该脚本激活"
set "_mOuwp=检测到Office 365/2016 UWP不支持使用该脚本激活"
...
if %_Debug% EQU 0 (
  ...
  echo.
  echo 调试模式运行中...
  if not defined _args (echo 窗口将在完成后关闭) else (echo 请稍后...)
  echo.
  echo 生成调试日志:
...
set "nKMS=不支持KMS激活..."
set "nEval=评估版本不能被激活。请安装完整版本的Windows OS."
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" set _eval=1
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*EvalEdition~*.mum" set "nEval=服务器评估版本不能被激活。请安装完整版本的Windows Server OS."
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*EvalCorEdition~*.mum" set _eval=1&set "nEval=服务器评估版本不能被激活。请转换为完整的Server OS."
set "_C16R="
...
if %ActOffice% EQU 0 (echo.&echo Office激活已关闭...)
...
if %ActWindows% EQU 0 (
  ...
  if %SSppHook% EQU 0 (
    ...
    echo.&echo 检查KMS激活ID^(s^)失败。 &call :CheckWS
    exit /b
    )
  )
)
...
if %OsppHook% EQU 0 (echo.&echo 未检测到已安装的Office %aword%产品...&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo 未检测到已安装的Office %aword%产品...&exit /b)
set err_offsvc=0
net start osppsvc /y %_Nul3% || (
sc start osppsvc %_Nul3%
if !errorlevel! EQU 1053 set err_offsvc=1
)
if %err_offsvc% EQU 1 (echo.&echo 错误：osppsvc服务未运行...&exit /b)
...
set ESU_ADD=0
call :keys %app%
if "%_key%"=="" (echo 未检测到相关的KMS客户端密钥&exit /b)
...
echo 产品激活成功，但是有效期没有增加。
if %OSType% EQU Win7 echo 详情请查看KB4487266。
exit /b
)
set Act_OK=0
...
if "%_ProductIds%"=="" (
if %_Office15% EQU 0 (echo 错误: 未检测到Office C2R产品IDs&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_LicensesPath%\ProPlus*.xrm-ms" (
if %_Office15% EQU 0 (echo 错误: 未检测到Office C2R许可证文件&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_Integrator%" (
if %_Office15% EQU 0 (echo 错误: 未检测到Office C2R许可证集成器&goto :%_fC2R%) else (goto :Reg15istry)
)
...
if "%_Product15Ids%"=="" (
if %_Office16% EQU 0 (echo 错误: 未检测到Office 2013 C2R产品IDs&goto :%_fC2R%) else (goto :CheckC2R)
)
...
if %_Office16% EQU 0 (echo 错误: 未检测到Office 2013 C2R许可证工具OSPP.vbs&goto :%_fC2R%) else (goto :CheckC2R)
)
...
echo.
echo Microsoft Office已使用vNext许可证激活。
)
if %sub_proj% EQU 1 (
  for %%a in (%_PrjSKU%) do set _%%a=0
echo.
echo Microsoft Project已使用vNext许可证激活。
)
if %sub_vsio% EQU 1 (
  for %%a in (%_VisSKU%) do set _%%a=0
echo.
echo Microsoft Visio已使用vNext许可证激活。
)
...
echo O365ProPlus 2016软件套装 ^<-^> Mondo 2016许可证
...
set _O365ProPlus=1
echo O365Business 2016软件套装 ^<-^> Mondo 2016许可证
...
set _O365ProPlus=1
echo O365SmallBusPrem 2016软件套装 ^<-^> Mondo 2016许可证
...
set _O365ProPlus=1
echo O365HomePrem 2016软件套装 ^<-^> Mondo 2016许可证
...
set _O365ProPlus=1
echo O365EduCloud 2016软件套装 ^<-^> Mondo 2016许可证
...
echo Professional 2019软件套装 -^> ProPlus%_ons% 许可证
call :InsLic ProPlus%_tag%
)
...
echo Professional 2016软件套装 -^> ProPlus%_ons% 许可证
call :InsLic ProPlus%_tag%
)
...
echo Standard 2019软件套装 -^> Standard%_ons% 许可证
...
echo Standard 2016软件套装 -^> Standard%_ons% 许可证
call :InsLic Standard%_tag%
)
...
if !_O365ProPlus! EQU 0 if !_ProPlus2021! EQU 0 if !_ProPlus2019! EQU 0 if !_ProPlus! EQU 0 if !_Professional2021! EQU 0 if !_Professional2019! EQU 0 if !_Professional! EQU 0 if !_Standard2021! EQU 0 if !_Standard2019! EQU 0 if !_Standard! EQU 0 (
  set _Standard2021=1
  echo %%a 2021软件套装 -^> Standard 2021 许可证
  call :InsLic Standard2021
  )
)
...
if !_O365ProPlus! EQU 0 if !_ProPlus2021! EQU 0 if !_ProPlus2019! EQU 0 if !_ProPlus! EQU 0 if !_Professional2021! EQU 0 if !_Professional2019! EQU 0 if !_Professional! EQU 0 if !_Standard2021! EQU 0 if !_Standard2019! EQU 0 if !_Standard! EQU 0 if !_%%a2021! EQU 0 (
  set _Standard2019=1
  echo %%a 2019软件套装 -^> Standard%_ons% 许可证
  call :InsLic Standard%_tag%
  )
)
...
if !_O365ProPlus! EQU 0 if !_ProPlus2021! EQU 0 if !_ProPlus2019! EQU 0 if !_ProPlus! EQU 0 if !_Professional2021! EQU 0 if !_Professional2019! EQU 0 if !_Professional! EQU 0 if !_Standard2021! EQU 0 if !_Standard2019! EQU 0 if !_Standard! EQU 0 if !_%%a2021! EQU 0 if !_%%a2019! EQU 0 (
  set _Standard=1
  echo %%a 2016软件套装 -^> Standard%_ons% 许可证
  call :InsLic Standard%_tag%
  )
)
...
echo O365ProPlus 2013软件套装 ^<-^> Mondo 2013 许可证
...
set _O365ProPlus=1
echo O365SmallBusPrem 2013软件套装 ^<-^> Mondo 2013 许可证
...
set _O365ProPlus=1
echo O365HomePrem 2013软件套装 ^<-^> Mondo 2013 许可证
...
set _O365ProPlus=1
echo O365Business 2013软件套装 ^<-^> Mondo 2013 许可证
...
if !_O365ProPlus! EQU 0 if !_ProPlus! EQU 0 if !_Professional! EQU 0 if !_Standard! EQU 0 (
  set _Standard=1
  echo %%a 2013软件套装 -^> Standard 2013 许可证
  call :Ins15Lic Standard
  )
)
...
echo 将不存在的IP地址10.0.0.10作为KMS服务器。
) else (
...
if not [%Act_OK%]==[1] (
echo.
echo 有任何问题, 请访问： https://10086678.xyz/troubleshoot
)
...
call :_color %_Red% "未连接网络。"
exit /b
)

if [%ERRORCODE%]==[-1073418124] (
echo 检查1688端口连接情况，该过程可能需要一定时间...
echo.
...
call :_color %Red% "您的网络未开放1688端口。"
echo.
echo 原因:   连接了受限网络[校园网等],
echo         或者防火墙阻止了连接。
echo.
echo 解决方案: 更换网络或使用离线KMS脚本
echo           https://github.com/abbodi1406/KMS_VL_ALL_AIO
) else (
echo 1688端口连接测试通过。
echo.
echo 请确保系统文件未被防火墙阻止。
echo 如果问题仍未解决, 测试以下离线KMS
echo https://github.com/abbodi1406/KMS_VL_ALL_AIO
)
echo.
)

echo 在这种情况下，KMS服务器不是问题。
exit /b
...
echo ## 注意 ##
echo.
echo 为了确保Office程序不显示你的许可证不是正版横幅，
echo 请运行激活选项一次，然后不要卸载。
...
echo __________________________________________________________________________________________
echo.
call :_color %Green% "在线KMS完全卸载已完成"
echo __________________________________________________________________________________________
)
...
echo 删除 [注册表信息] HKCR\DesktopBackground\shell\Activate Windows - Office
...
echo.
echo 信息:
echo 激活将在有网络连接时每两周续期一次。
echo 将只更新已安装的KMS许可证。
echo __________________________________________________________________________________________
echo.
if defined ActTask (
call :_color %Green% "续期和激活任务创建成功。"
) else (
call :_color %Green% "续期任务创建成功。"
)
echo.
call :_color %Gray% "请确保至少运行了一次激活选项。"
echo __________________________________________________________________________________________
)
...
echo:
echo   在线KMS激活脚本是MAS的一部分。
echo:   
echo   首页: 10086678.xyz
...
if [%1]==[act_win] if %_kms38% EQU 1 (
echo Windows已使用KMS38激活
exit /b
)

if %errorcode% EQU 12345 (
echo 产品激活失败
echo 因受限/无网络连接，无法测试KMS服务器
set _tserror=1
exit /b
)
...
if [%1]==[act_win] if %winbuild% LSS 9200 echo 这台计算机的OEM BIOS不符合要求，因此无法通过KMS激活Windows 7。
exit /b
)

if %errorcode% EQU -1073417728 (
echo 产品激活失败: 0xC004F200
echo Windows需要重建激活文件。
set _tserror=1
exit /b
)
...
cscript //nologo slmgr.vbs /dli || (echo 执行slmgr.vbs&del /f /q slmgr.vbs错误&popd&goto :casVend)
...
if %GracePeriodRemaining% EQU 0 (
  if %winID% EQU 1 (set "ExpireMsg=设备已永久激活。") else (set "ExpireMsg=产品已永久激活。")
  ) else (
  set "LicenseMsg=%_mTag% 激活剩余时间: %GracePeriodRemaining% 分钟 (%_gpr% 天)"
  if defined _xpr set "ExpireMsg=%_mTag% 激活过期时间 %_xpr%"
  )
)
...
echo 请激活产品以更新KMS客户端信息值。
exit /b
)
echo 最近的激活信息：
echo 密钥管理服务客户端信息
echo.    客户端机器ID（CMID）： %ClientMachineID%
echo.    %KmsDns%
echo.    %KmsReg%
...
echo.    KMS主机缓存： %KeyManagementServiceHostCaching%
if defined KeyManagementServiceLookupDomain echo.    KMS SRV记录查找域： %KeyManagementServiceLookupDomain%
if defined ExpireMsg echo.&echo.    %ExpireMsg%
exit /b
...
if %_erl%==7 exit /b
if %_erl%==6 start https://10086678.xyz/fix-wpa-registry.html &goto at_menu
if %_erl%==5 goto:retokens
if %_erl%==4 goto:fixwmi
if %_erl%==3 goto:sfcscan
if %_erl%==2 goto:dism_rest
if %_erl%==1 start https://10086678.xyz/troubleshoot.html &goto at_menu
goto :at_menu
...
%eline%
echo 检测到不支持的操作系统版本。
echo 该命令仅支持Windows 8/8.1/10/11及其服务器等效版本。
goto :at_back
)
...
call :_color2 %_White% "     " %Red% "检查网络状态  [未连接]"
)

echo %line%
echo:
echo      DISM使用Windows Update提供所需的文件来修复损坏。
echo      这将需要5-15分钟或更长时间。
echo %line%
echo:
echo      Notes:
echo:
call :_color2 %_White% "     - " %Gray% "请确保已连接到网络。"
call :_color2 %_White% "     - " %Gray% "请确保Windows更新正常工作。"
echo:
echo %line%
echo:
...
echo:
call :_color %Gray% "CBS和DISM日志已复制到桌面上的AT_Logs文件夹中。"
goto :at_back
...
echo %line%
echo:    
echo      系统文件检查器将修复缺失或损坏的系统文件。
echo      这将需要10-15分钟或更长时间。
echo:
echo      如果SFC无法修复某些问题，则再次运行该命令以查看是否可能在下一次运行时能够修复。 
echo      有时可能需要运行sfc /scannow命令3次，
echo      在每次重启计算机后才能完全修复它所能修复的所有内容。
echo:   
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回 : "
...
echo:
call :_color %Gray% "CBS日志已复制到桌面上的AT_Logs文件夹中。"
goto :at_back
...
echo:
echo       - 此选项将，
echo            - 取消激活Windows和Office，您可能需要重新激活。
echo            - 清除ClipSVC、Office vNext、SPP和OSPP许可证。
echo            - 修复tokens文件夹和注册表的SPP权限。
echo            - 触发Office的修复选项。
echo:
call :_color2 %_White% "      - " %Red% "只有在必要时才应用它。"
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回 : "
...
if %winbuild% LSS 10240 (
echo 只有在Windows 10/11和相应的服务器上才支持ClipSVC许可证重建。
echo 跳过...
goto :cleanvnext
)
...
echo:
echo 正在重建文件夹 %ProgramData%\Microsoft\Windows\ClipSVC\
net start ClipSVC /y %nul%
timeout /t 3 %nul%
...
echo %line%
echo:
call :_color %Magenta% "正在重建 SPP 许可证令牌。"
echo:
...
if defined token (
echo:
call :_color %Red% "删除.dat files失败。"
echo:
)
...
echo:
if not defined token (
call :_color %Red% "重建tokens.dat文件失败。"
) else (
...
if defined token (
echo:
call :_color %Red% "删除.dat文件失败。"
echo:
)

echo:
echo 启动osppsvc服务以生成tokens.dat
...
echo 您需要在Windows设置中使用修复选项来修复Office。
echo:
...
echo:
call :_color %_Yellow% "将弹出一个窗口，在该窗口中，您需要选择[快速修复]修复选项..."
...
if defined uwp16 (
echo:
echo 注意：跳过对Office 16.0 UWP的修复。
echo       您需要在Windows设置中使用重置选项来进行操作。
echo ________________________________________________________________
echo:
start ms-settings:appsfeatures
)
...
if defined c2r14 (
echo:
echo 注意：跳过对Office 14.0 C2R的修复。
echo       您需要在Windows设置中使用修复选项来进行操作。
echo ________________________________________________________________
echo:
start appwiz.cpl
)
...
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo 不建议在Windows Server上重新构建WMI。 中止...
goto :at_back
)

for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
%eline%
echo 未检测到wmic.exe。 中止...
goto :at_back
)
...
call :_color %Red% "[失败]"
echo:
call :_color %Magenta% "建议选择[重新启动]选项，然后再次应用“修复WMI”选项。"
echo %line%
echo:
...
echo:
if not "%regSKU%"=="%wmiSKU%" (
echo 注意：SKU值不同- WMI:%wmiSKU% Reg:%regSKU%
echo:
)
call :dk_color %_Green% "[1]: 继续   [0]: %_exitmsg%"
choice /C:01 /N
...
echo 更多帮助： https://10086678.xyz/troubleshoot
)
%line%
...
echo 当前版本 [%osedition% ^| %winbuild%] 无法更改为其他版本。
%line%
goto ced_done
)
...
%line%
echo:
call :dk_color %Gray% "您可以将版[%osedition%] [%winbuild%] 更改为以下版本之一。"
%line%
echo:
...
echo:
if defined note (
echo CBS Upgrade Method 可用于紫色的版本。
echo:
)
call :dk_color %_Green% "请输入菜单选项, 并按"Enter"键:"
set /p inpt=
...
%eline%
echo [%osedition%] 无法更改为 [%targetedition%] 版本，因为缺少非 OEM 密钥。
echo 非 OEM 密钥是必须的，才能从核心版更改为非核心版。
goto ced_done
)
)
...
if not [%%#]==[] (
%eline%
echo 需要断开互联网连接才能将版本 [%osedition%] 更改为 [%targetedition%]
echo 断开互联网连接，然后按任意键...
pause >nul
goto ced_loop
)
...
echo:
echo 更改当前版本 [%osedition%] 为 [%targetedition%]
echo:

if %_changepk%==1 (
call :dk_color %Magenta% "Notes-"
echo:
echo  - 如果升级窗口中出现错误，您可以安全地忽略它，
echo    但在这种情况下，您必须手动重新启动系统。
echo:
echo  - 保存您的工作，然后继续操作，系统将会自动重新启动。
echo  - 系统重启后您可以连接到互联网。
echo:
echo  - 一旦更改了Windows版本，您需要使用HWID选项进行激活。
echo:
...
call :dk_color %Green% "[成功]"
echo:
call :dk_color %Gray% "需要重新启动系统以正确更改版本。"
) else (
call :dk_color %Red% "[失败] [错误代码: 0x!=ExitCode!]"
)
)
...
echo:
echo 更换当前版本 [%osedition%] 至 [%targetedition%]
echo:
call :dk_color %Magenta% "重要 - 保存你的任务，系统将自动重启。"
if %winbuild% GEQ 17034 if %targetedition%==Professional echo           - 即将安装企业版密钥以取代专业版密钥，后续可以快速更换至专业版。
echo:
