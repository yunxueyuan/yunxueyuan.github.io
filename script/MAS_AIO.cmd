...
if %_erl%==8 exit /b
if %_erl%==7 start %Homepage% & goto :MainMenu
...
call :dk_color %Magenta% "下载地址： %Homepage%"
)
)
...
call :dk_color2 %Magenta% "更多帮助：" %_Yellow% " %Homepage%"
)
)
...
::  5th column = Ticket signature value. It's as it is, it's not encoded. (Visit %Homepage% to see how it's generated)
...
if %winbuild% GEQ 9200 (
  if %OffUWP% EQU 0 (echo.&echo 没有检测到已安装的 Office 2013-2021 产品...) else (echo.&echo %_mOuwp%)
  exit /b
  )
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo 没有检测到已安装的 Office  %aword% 产品...&exit /b))
)
